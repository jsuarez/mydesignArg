...
<form id="form-portfoliowork" method="post" action="<?=site_url('/jpanel/portfolio/'.(isset($info) ? 'workedit' : 'workcreate'))?>" enctype="application/x-www-form-urlencoded" class="form-portfolio">
    <div class="trow">
        <label class="label" for="txtThumb">* Im&aacute;gen</label>
<?php
$src = @$info['filename']!='' ? UPLOAD_PATH_PORTFOLIOWORKS . @$info['filename'] : '';
?>
        <div id="cont-image-1" class="span-13 last">
            <img src="<?=$src?>" alt="<?=@$info['filename']?>" width="<?=IMAGESIZE_WIDTH_PORTFOLIOWORK?>" height="<?=IMAGESIZE_HEIGHT_PORTFOLIOWORK?>" class="ajaxupload-thumb fleft framethumb <?php if( $src=='' ) echo 'hide'?>" />
    ...
    <div class="trow">
        <label class="label" for="cboType">Tipo</label>
        <div class="span-13 last">
            <select name="cboType" id="cboType">
                <option value="web" <?php if( @$info['type']=="web" ) echo 'selected="selected"'?>>Web</option>
                <option value="grafica" <?php if( @$info['type']=="grafica" ) echo 'selected="selected"'?>>Grafica</option>
            </select>
        </div>
    </div>
    <div class="trow">
        <label class="label" for="txtUrl">URL</label>
        <div class="span-13 last"><input type="text" name="txtUrl" id="txtUrl" value="<?=@$info['url']?>" /></div>
    </div>
...
<form id="ajaxupload-form" action="<?=site_url('/jpanel/portfolio/ajax_upload_products')?>" method="post" enctype="multipart/form-data" target="ifr" class="hide">
    ...
    <input type="hidden" name="path" value="<?=UPLOAD_PATH_PORTFOLIOWORKS?>" />
    <input type="hidden" name="width" value="<?=IMAGESIZE_WIDTH_PORTFOLIOWORK?>" />
    <input type="hidden" name="height" value="<?=IMAGESIZE_HEIGHT_PORTFOLIOWORK?>" />
</form>